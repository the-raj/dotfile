...
exports.failedToParseTasksJson = "Failed to parse tasks.json, possibly due to comments or trailing commas.";
...
function getRawTasksJson() {
    ...
    return new Promise((resolve, reject) => {
        fs.exists(path, exists => {
            ...
            fileContents = fileContents.replace(/^\s*\/\/.*$/gm, "");
            let rawTasks = {};
            try {
                rawTasks = JSON.parse(fileContents);
            }
            catch (error) {
                return reject(new Error(exports.failedToParseTasksJson));
            }
            resolve(rawTasks);
        });
    });
...
function ensureBuildTaskExists(taskName) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const buildTasks = yield extension_1.getBuildTasks(false);
